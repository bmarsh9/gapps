...
class TenantRole(BaseEnum):
    ...
    @classmethod
    def get_role_names_with_access_to_tenant(cls) -> List[str]:
        roles_with_access = [cls.ADMIN, cls.EDITOR, cls.VIEWER, cls.USER]
        return [role.value for role in roles_with_access]
    
    @classmethod
    def get_role_names_that_can_manage_tenant(cls) -> List[str]:
        roles_with_access = [cls.ADMIN, cls.EDITOR]
        return [role.value for role in roles_with_access]

    
class ProjectRoles(BaseEnum):
    VIEWER = 'viewer'
    ...
    @classmethod
    def get_roles_that_can_update_project_notes(cls) -> List[str]:
        roles_with_access = [cls.CONTRIBUTOR, cls.MANAGER, cls.AUDITOR]
        return [role.value for role in roles_with_access]
    
    @classmethod
    def get_roles_that_can_update_project_comments(cls) -> List[str]:
        roles_with_access = [cls.CONTRIBUTOR, cls.MANAGER, cls.AUDITOR]
        return [role.value for role in roles_with_access]

    @classmethod
    def get_roles_that_can_update_project_settings(cls) -> List[str]:
        roles_with_access = [cls.MANAGER]
        return [role.value for role in roles_with_access]
    
    @classmethod
    def get_roles_that_can_manage_project_members(cls) -> List[str]:
        roles_with_access = [cls.MANAGER]
        return [role.value for role in roles_with_access]
